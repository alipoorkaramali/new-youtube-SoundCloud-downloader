...
خروج سریع وقتی پست جدیدی نیست (اتوماتیک ساعتی)
"""
...
class DebugTelegramChannelScraper(TelegramChannelScraper):
    ...
    async def _run_impl(self):
        ...
        # در حالت auto با stop_before، دورهای زیاد لازم نیست
        ...
        while len(all_items) < self.limit and rounds < max_rounds:
            ...
            # در auto با stop_before هرگز id قدیمی‌تر حدس نزن
            if not items and rounds > 1 and all_items and not getattr(self, "stop_before_id", None):
        ...
        try:
            gen = OutputGenerator(self.base_dir, self.channel, all_items, {}, debug_mode=True, append_mode=True)
            gen.run_all()
        except Exception as e:
